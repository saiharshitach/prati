...
public class DoMalogServisaIzvestaj extends PrintPreviewReport<Vozila>{
	...
	public DoMalogServisaIzvestaj(ArrayList<Objekti> objekti, int tipServisa, int doServisa) {
		...
		.setTitle("Преглед возила за мали сервис")
		.setGrandTotalLegend("укупно")
		.addAutoText("Преглед возила за мали сервис за мање од " + doServisa + "км" , AutoText.POSITION_HEADER, AutoText.ALIGMENT_LEFT, 450, headerStyle)
		...
		.addColumn(ColumnBuilder.getNew()
				...
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("maliPoslednjiDatum", Date.class)
				.setTitle("датум")
				.setStyle(datum)
				.setWidth(25)
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("danaOdMs", Integer.class)
				.setTitle("дана")
				.setStyle(broj)
				.setWidth(12)
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("maliPoslednjiGPSkm", Float.class)
				.setTitle("урађен на ГПС км")
				.setStyle(broj)
				//.setWidth(30)
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("kmOdGpsMs", Float.class)
				.setTitle("ГПС км од сервиса")
				.setStyle(broj)
				//.setWidth(20)
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("maliPoslednjiOBDkm", Integer.class)
				.setTitle("урађен на ОБД км")
				//.setWidth(10)
				.setStyle(broj)
				.build())
		.addColumn(ColumnBuilder.getNew()
				.setColumnProperty("kmOdObdMs", Integer.class)
