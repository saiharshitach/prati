...
public class UkupniTroskoviLayout extends VerticalLayout{
	...
	public UkupniTroskoviLayout(ArrayList<Objekti> vozila, Timestamp datumVremeOd, Timestamp datumVremeDo, Integer tipTroska) {
		...
		izvestaj.downloadPdfOnClick(pdf, "ukupni_troskovi_" + datumVreme.format(new Date()) + ".pdf", izvestaj.vratiSeriju());
		izvestaj.downloadDocxOnClick(doc, "ukupni_troskovi_" + datumVreme.format(new Date()) + ".doc", izvestaj.vratiSeriju());
		izvestaj.downloadXlsOnClick(xls, "ukupni_troskovi_" + datumVreme.format(new Date()) + ".xlsx", izvestaj.vratiSeriju());
