...
public class VozaciPasosiView extends OpstiView implements OpstiViewInterface{
	...
	public void buildTable() {
		...
		tabela.addColumn(vozaciPasos -> vozaciPasos.getVozaci() == null ? "" : vozaciPasos.getVozaci().toString()).setCaption("корисник");
		...
		if(isSistem() || (korisnik.isAdmin() && korisnik.getOrganizacija() == null)) {
					tabela.addColumn(vozaciPasos -> vozaciPasos.getVozaci() == null ? "" : vozaciPasos.getVozaci().getOrganizacija() == null ? "" : 
						vozaciPasos.getVozaci().getOrganizacija().getNaziv()).setCaption("организација");
		}
		if(isSistem()) {
	...
	public void ukloniPodatak() {
		if(izabrani != null) {
			if(!izabrani.isIzbrisan()) {
				...
				pokaziPorukuUspesno("подаци за пасош " + izabrani.getVozaci().toString() + " избрисани");
			}else {
